...
class Dealer extends Controller
{
	...
	public function dealerHomeRequest($userid, $password, $msisdn, $input, $session_id, $identification_customer_type)	
	{
		...
		if($array_count_information==1)
		{
			...
			$notification .= "3. R/D Balance Check"."\n";
	...
	{		
		...
		if(count($cust_data)>0)
		{
			...
			if(!empty($new_acc_no))
			{
				...
				$text="Registration Successful DSR Account ID No: ".$new_acc_no;
				$ccObj->send_message($dealer_number, $text);					
