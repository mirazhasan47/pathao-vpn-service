...
class DSR extends Controller
{
	...
	public function DSRHomeRequest($userid, $password, $msisdn, $input, $session_id, $identification_customer_type)	
	{
		...
		if($array_count_information==1)
		{
			...
			$notification .= "3. Retailer Balance Check"."\n";
		...
		elseif($array_count_information==2)
		{
			...
			{
				//Registration
				$result['notification']="Use Apps for registration";
				$result['session']='FB';
				return $result;
			}
			elseif($mainMenu==3)
			{
				...
				$result['notification']="Enter Retailer Number";
				$result['session']='FC';
				return $result;
			}
			elseif($mainMenu==4)
		...
		elseif($array_count_information==4)
		{
			...
			else
			{
				...
				return $result;*/
				$result['notification']="Upcomming Service......";
				$result['session']='FB';
				return $result;	
			}
		}
	...
	{		
		...
		if($cust_data)
		{
			...
			//--Connection---
			$connections =  DB::table('customers')->select(DB::raw('COUNT(id) AS ttl'))->where('dsr_id', $acc_no)->where(DB::raw('DATE(created_at)'),  $current_date)->first();			
			if($connections)
			{
	...
	{		
		...
		if(count($checkdata)==0)
		{
			...
			if(count($cust_data)>0)
			{
				...
				$dealer_acc_no=$cust_data[0]->parent_id;

				$dsr_id=$cust_data[0]->acc_no;
				$dsr_pre_balance=$cust_data[0]->balance;
				$dsr_new_balance=0;
				$dsr_number=$cust_data[0]->mobile_no;;
				...
				if($dsr_pre_balance>$fee)
				{
					...
					if(!empty($new_acc_no))
					{
						...
						$custData['nid']=$nid;
						$custData['customer_type_id']=7;
						$custData['parent_id']=$dealer_acc_no;
						$custData['dsr_id']=$dsr_id;
						...
						{
							$sender=$dsr_id;
							$sender_pre_balance=$dsr_pre_balance;
							$sender_new_balance=$sender_pre_balance-$dealer_profit;
							$dsr_new_balance=$sender_new_balance;
							...
							$number=$ccObj->getCustPhoneByAcc($dealer_id);
							$text="Registration Successful Retailer Account ID No: ".$new_acc_no.", Your Account Opening comm: ".$dealer_profit." Tk Thank you for use PayStation";
						...
						$text="Registration Successful Retailer Account ID No: ".$new_acc_no.", Account Opening fee ".$fee."/- Deducted from your account. Your new balance is Tk.".$dsr_new_balance;
						$ccObj->send_message($dsr_number, $text);
	...
	{		
		...
		$cust_data = DB::table('customers')->select('acc_no','balance')->where('mobile_no', $msisdn)->where('pin', $pin)->limit(1)->first();		
		if($cust_data)
		{
			...
			if($rcverData)
			{
				...
				if(!$ccObj->checkTransferBetweenTwoUserValidOrNot($sender, $receiver))
				{
					$result['notification']="You are not allowed to transfer balance to this account.";
					$result['session']='FB';
					return $result;
					exit();
				}
				...
				else
				{
					...
					if($sender_pre_balance>=$amount)
					{
						...
						$rs=$ccObj->send_message($number, $text);
	...
	{		
		...
		$cust_data = DB::table('customers')->select('parent_id','acc_no','balance')->where('mobile_no', $msisdn)->where('pin', $pin)->limit(1)->first();		
		if($cust_data)
		{
			$receiver = $cust_data->parent_id;
			$sender = $cust_data->acc_no;
			$sender_pre_balance=$cust_data->balance;
			$sender_new_balance=$sender_pre_balance-$amount;
			...
			if($rcverData)
			{
				...
				if($sender_pre_balance>=$amount)
				{
					...
					$senderData = $ccObj->getCustInfoByAccNo($sender);
					$sender_cust_id = $senderData[0]->id;
					$debitDetails = "Refund Balance ".$amount." to Account No ".$receiver.", Your current balance ".$sender_new_balance;
					$ccObj->createCustomerStatement($sender_cust_id, $sender, $sender_pre_balance, $amount, 2, 3, 6, 'Wallet Refund', 'USSD', $debitDetails, $requestId, 'transaction');
					// Code For Customer Statement

					// Code For Customer Statement
					$receiverData = $ccObj->getCustInfoByAccNo($receiver);
					$receiver_cust_id = $receiverData[0]->id;
					$creditDetails = "Wallet Refund Tk ".$amount." from Account No ".$sender.", Your current balance ".$receiver_new_balance;
					$ccObj->createCustomerStatement($receiver_cust_id, $receiver, $receiver_pre_balance, $amount, 1, 3, 6, 'Wallet Refund', 'USSD', $creditDetails, $requestId, 'transaction');
					// Code For Customer Statement
